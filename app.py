...
if st.button("✨ Visualize & Narrate Wallet", type="primary"):
    ...
    # 🔥 NEW: Map numbers to fictional concepts based on Genre
    prompt = f"""
    You are a legendary sci-fi author. 
    
    **THE DATA:**
    Subject Persona: "{vibe_name}" ({vibe_desc})
    Genre: {genre}
    Transactions: {json.dumps(tx_data)}
    
    **THE RULES:**
    1. DO NOT mention "TRX" or "Blockchain" or "Wallet". 
    2. TRANSLATE the data into a fiction:
       - Small transaction = Buying food, ammo, or data chips.
       - Large transaction = Buying a ship, a weapon, or paying a ransom.
       - Empty wallet = Starvation or hiding.
    3. Make it VISUAL.
    
    **OUTPUT FORMAT (Strict):**
    SCENE 1: [Write 2 sentences of story. Make the transaction a specific plot point.]
    IMAGE_PROMPT 1: [Detailed visual description of that scene, strictly in {genre} style]
    
    SCENE 2: [Write 2 sentences continuing the story.]
    IMAGE_PROMPT 2: [Detailed visual description of that scene, strictly in {genre} style]
    """
