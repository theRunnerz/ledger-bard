...
# ---------------------------------------------------------
# 4. MAIN EXECUTION (The "Director Mode") 🎬
# ---------------------------------------------------------
if st.button("✨ Visualize & Narrate Wallet", type="primary"):
    
    # 1. FETCH DATA (Must happen first!)
    with st.spinner("🔍 Scanning Blockchain History..."):
        tx_data = get_transfers(wallet_address)
    
    # 2. ANALYZE PERSONA (The "Brain")
    vibe_name, vibe_desc = analyze_wallet_vibe(tx_data)
    
    # Display the Persona Badge
    st.success(f"🎭 **Persona Detected:** {vibe_name}")
    st.caption(f"_{vibe_desc}_")

    # 3. EXTRACT DEEP DETAILS (The "Texture")
    # We define these variables safely so the script doesn't crash if data is empty
    if tx_data and len(tx_data) > 0:
        # TRON data uses 'value' (integers), sometimes it's formatted as string
        try:
            raw_val = float(tx_data[0].get('value', 0))
            # If it's TRON (6 decimals), convert to human readable
            last_tx_amount = raw_val / 1000000 if raw_val > 1000 else raw_val
    ...
    # Define the "Stakes" based on the amount
    ...
    # 4. THE DEEP DIVE PROMPT (The "Novelist")
    prompt = f"""
    ...
    **GENERATE 3 SCENES (Beginning, Climax, Aftermath):**
    ...
    [The exchange happens. Verification. The tension peaks. The specific amount **{last_tx_amount}** must appear on a screen/holo/scroll. Is it a betrayal? Success?]
    IMAGE_PROMPT 2:
    ...
    IMAGE_PROMPT 3:
    [{genre} style. Low angle. The character walking away into the distance/fog/light. Text "TRANSACTION COMPLETE" visible vaguely in the background.]
    """

    # 5. GENERATE CONTENT
    model = GenerativeModel("gemini-2.0-flash-lite-001")
    
    with st.spinner("🤖 Writing the script & Directing scenes..."):
        response = model.generate_content(prompt)
        full_text = response.text

    # 6. PARSING LOOP (The "Comic Book" Layout)
    lines = full_text.split('\n')
    
    # Store audio for unique layout
    current_audio = None
    ...
    for line in lines:
        ...
        if clean_line.upper().startswith("SCENE"):
            ...
            # Generate Audio
            current_audio = speak_story(display_text, genre)
            ...
            with st.spinner(f"🎨 Painting scene..."):
                ...
                if img:
                    ...
                    with col2:
                        ...
                        if current_audio:
                             st.audio(current_audio, format="audio/wav")
                        else:
                            st.caption("No audio for this scene.")